...
class DepressionSpider(CrawlSpider):
    ...
    rules = [
           ...
           # callback='parse_home'),
            ...
            Rule(SgmlLinkExtractor(allow=('/group/topic/[^/]+/\?start'),restrict_xpaths=('//span[@class="next"]')), callback="parse_reply", follow=True), # this rule is
    ...
    def parse_post(self, response):
       ...
       # itemP['author'] = self._get_ID_from_member_URL("".join(authorURL)) # type change from list to string
        ...
        itemP['content_P'] = response.xpath('//div[@class="topic-content"]/p/text()').extract()
        num = response.xpath('//span[@class="fav-num"]/a/text()').re("\d+")
        if (num):
            itemP['numberLikes_P'] = num[0]
        else:
